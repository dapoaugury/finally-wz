...
def findcompare(request):
    ...
    # auguried - 20170319 - Endowment Products

    if category == '4': # Endowment
        if 'limited_pay_term' in request.POST:
            limited_pay_term = request.POST['limited_pay_term']
            sc_limited_pay_term = u'"20":%s' % (limited_pay_term)
        if 'policy_term' in request.POST:
            policy_term = request.POST['policy_term']
            sc_policy_term = u'"19":%s' % (policy_term)
        if 'maturity_amount' in request.POST:
        ...
        sc_gender = u'"4":357' # Male 
        sc_radio_smoker = u'"13":474' # n
        variants = ProductVariant.objects.filter( Q(attributes__contains=sc_limited_pay_term) & Q(attributes__contains=sc_policy_term) & Q( product_id__in = Product.objects.filter( Q(categories = category ) ).values_list('pk') ) ).                     select_related('product').order_by('-yield_option_1')
...
def getrecommendation(request):
    from .product.templatetags.auguri import display_attribute_id, display_endowment_premium


    local_host = get_local_host(request)
    logger.debug('Debug Message: getrecommendation request.POST=%s', request.GET)

    base_ctx = {'request': request, 'next': next, 'data': request.GET}
    logger.debug('Debug Message getrecommendation base_ctx=%s', base_ctx)
    logger.addFilter(ContextFilter())
    ...
    age = request.GET['age']
    sc_age = u'"3":%s' % (display_attribute_id(3, age))
    sc_gender = u'"4":%s' % (display_attribute_id(4, gender))
    logger.debug('Debug Message getrecommendation sc_gender=%s, age=%s', sc_gender, sc_age)
    ...
    if category == '3':
        if 'montly_retire_income' in request.GET:
            ...
            sc_montly_retire_income = u'"12":%s' % (display_attribute_id(12, montly_retire_income))
        if 'payout_age' in request.GET:
            payout_age = request.GET['payout_age']
            sc_payout_age = u'"11":%s' % (display_attribute_id(11, payout_age))
        ...
        logger.debug('Debug Message getrecommendation sc_montly_retire_income=%s, sc_payout_age=%s', sc_montly_retire_income, sc_payout_age)
    ...
    if category == '4': # Endowment
        # if 'limited_pay_term' in request.GET:
        #     limited_pay_term = request.GET['limited_pay_term']
        #     sc_limited_pay_term = u'"20":%s' % (limited_pay_term)
        if 'policy_term' in request.GET:
            policy_term = request.GET['policy_term']
            sc_policy_term = u'"19":%s' % (display_attribute_id(19, policy_term))
        if 'maturity_amount' in request.GET:
            maturity_amount = request.GET['maturity_amount']
        base_ctx = {'request': request, 'next': next, 'data': request.POST, 'variants': None, 'variants_total': 0}
        logger.debug('Debug Message: getrecommendation sc_policy_term=%s, base_ctx=%s', sc_policy_term, base_ctx)

        # auguried - 20161223 - Ignore gender and smoker 
        sc_gender = u'"4":357' # Male 
        sc_radio_smoker = u'"13":474' # n
        variants = ProductVariant.objects.filter( Q(attributes__contains=sc_policy_term) & Q( product_id__in = Product.objects.filter( Q(categories = category ) ).values_list('pk') ) ).select_related('product').order_by('-yield_option_1')
    ...
    for variant in variants:
        ...
        for attribute in variant.attributes:
            display[attribute] = AttributeChoiceValue.objects.filter(Q (attribute_id = attribute), id = variant.attributes[attribute])[0]
            logger.debug('Debug Message: getrecommend attribute=%s', attribute)
            dbg_i = dbg_i + 1
        ...
        if category == '4':
            ...
            if variant.get_yield_option_1_per_item() is None:
                irr = 0
            else:
                irr = float(variant.get_yield_option_1_per_item())
            # maturity_amount
            limited_pay_period = "10"
            # logger.debug('Debug Message: getrecommend yearly_premium=%s', display[attribute])
            payout_period = policy_term
            yearly_premium = display_endowment_premium(irr, maturity_amount, limited_pay_period, payout_period)
            yearly_premium_price = Price(net=yearly_premium, gross=yearly_premium, currency=settings.DEFAULT_CURRENCY)
            logger.debug('Debug Message: getrecommend yearly_premium=%s', yearly_premium_price)

            json_list.append({ "id": variant.id, "name": product.name,"get_price_per_item": yearly_premium,"get_price_option_1_per_item": variant.get_price_option_1_per_item(),"get_price_option_2_per_item": variant.get_price_option_2_per_item(), "description":product.description,"usp1":product.usp1,      "usp2":product.usp2,"usp3":product.usp3,"url": url,"attributes": variant.attributes })
        else:
...
def getresult(request):
    local_host = get_local_host(request)
    user = {'name': request.GET['name'], 'smoker':request.GET['smoker'], 'sex':request.GET['sex'], 'birthday':request.GET['birthday'], 'age_retire':request.    GET['age_retire']}
